...
#define ASTABLE_GPIO GPIO_NUM_2
#define MONO_GPIO GPIO_NUM_4
#define PWM_GPIO GPIO_NUM_5
#define MONO_BTN_GPIO GPIO_NUM_0  // BOOT button
...
void blink_task(void *pvParameter) {
    gpio_set_direction(ASTABLE_GPIO, GPIO_MODE_OUTPUT);
    while (1) {
        if (blink_enabled) {
            gpio_set_level(ASTABLE_GPIO, 1);
            vTaskDelay(pdMS_TO_TICKS(blink_period_ms / 2));
            gpio_set_level(ASTABLE_GPIO, 0);
            vTaskDelay(pdMS_TO_TICKS(blink_period_ms / 2));
        } else {
...
// MONOESTABLE
void mono_task(void *pvParameter) {
    gpio_set_direction(MONO_GPIO, GPIO_MODE_OUTPUT);
    gpio_set_pull_mode(MONO_BTN_GPIO, GPIO_PULLUP_ONLY);
    while (1) {
        ...
        if (level == 1 && btn_pressed) {
            btn_pressed = false;
            int64_t duration = (now - btn_press_time) / 1000;
            ...
            snprintf(mono_msg, sizeof(mono_msg),
                     "El botón fue presionado por %.2f segundos", duration / 1000.0);
        }
...
esp_err_t root_get_handler(httpd_req_t *req) {
    ...
    snprintf(html, sizeof(html),
        ...
        "body { font-family:sans-serif; padding:2rem; text-align:center; background:black; color:red; }"
        "input, select, button { padding:15px; margin:15px; width:80%%; max-width:300px; font-size:18px; border-radius:5px; }"
        "button { background:#dc3545; color:white; border:none; cursor:pointer; }"
        "button:hover { background:#c82333; }"
        "</style></head><body>"
        "<h2>🌀 Astable - GPIO 2</h2>"
        "<input type='number' id='resistencia' placeholder='Resistencia (Ω)' step='any'><br>"
        "<input type='number' id='capacitancia' placeholder='Capacitancia' step='any'><br>"
        "<select id='unidad'><option value='uF'>uF</option><option value='F'>F</option></select><br>"
        ...
        "<h2>🟢 Monoestable - GPIO 4 (LED) / BOOT (Botón)</h2>"
        "<p>%s</p>"
        "<button onclick='ejecutarMono()'>Ejecutar</button>"
        "<script>"
        ...
        "if(unidad==='uF'){C=C/1000000;}let T=2*R*C;let f=1/T;let ms=T*1000;"
...
// URI - MONOESTABLE
esp_err_t ejecutar_mono_get_handler(httpd_req_t *req) {
    snprintf(mono_msg, sizeof(mono_msg), "El botón ha sido ejecutado.");
    httpd_resp_send(req, NULL, 0);
    return ESP_OK;
}
...
httpd_handle_t start_webserver(void) {
    ...
    if (httpd_start(&server, &config) == ESP_OK) {
        ...
        httpd_uri_t ejecutar_mono = { .uri = "/ejecutar_mono", .method = HTTP_GET, .handler = ejecutar_mono_get_handler };
        httpd_register_uri_handler(server, &root);
        httpd_register_uri_handler(server, &set);
        httpd_register_uri_handler(server, &pwm);
        httpd_register_uri_handler(server, &ejecutar_mono);
    }
    return server;
}
...
void app_main(void) {
    ...
    start_webserver();
    setup_pwm(pwm_duty);
    gpio_reset_pin(ASTABLE_GPIO);
    gpio_set_direction(ASTABLE_GPIO, GPIO_MODE_OUTPUT);
    gpio_reset_pin(MONO_GPIO);
    gpio_set_direction(MONO_GPIO, GPIO_MODE_OUTPUT);
    gpio_reset_pin(MONO_BTN_GPIO);
    gpio_set_direction(MONO_BTN_GPIO, GPIO_MODE_INPUT);
    xTaskCreate(blink_task, "blink_task", 2048, NULL, 5, NULL);
    xTaskCreate(mono_task, "mono_task", 2048, NULL, 5, NULL);
}
